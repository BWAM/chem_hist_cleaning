...
setdiff(temp_join$match,temp_result$match)
...
qa_mod3<-qa_mod2 %>% 
  ...
  mutate(validator_qualifiers=case_when(#BWRM
       ...
       sample_delivery_group=="R2207613" & ~"R",
  ...
  #####################################fix flag narrative#######################
    mutate(interpreted_qualifiers=case_when(
      ...
      ample_delivery_group=="R2207977" & chemical_name=="zinc"~"Equipment Blank error",
...
dups <- result.all.flags.full %>% 
  ...
  mutate(dups = n()) %>% 
  filter(dups > 1)
 # Found: nothing for hansgrrot or pws projects 2022
...
# Found: Eighteenmile PCBs, lake sediment samples, and tech park DI blank. OK. nothing for pws
...
# Import pcodes 
pcode.join <- read_csv(file.path(datamod.dir, "Final_Chemistry_ITS", "pcode_join_table", "dec_pcode_join_2022-03-08.csv")) %>%
...
write.csv(event.id,here::here("data/2022/event_id_2022_hansgroot.csv"))
...
write.table(sample.all.FINAL_cor, file = file.path(target.dir, "SMAS_sawmill.csv"),sep=",", row.names = FALSE, na = "")
write.table(result.all.FINAL, file = file.path(target.dir, "SMAS_chem_2022_RESULT_pws.csv"),sep=",", row.names = FALSE, na = "")
