...
pcode <- read.csv(file.path(datamod.dir, "20201013_S_CHEM_PARAMETER.csv"), stringsAsFactors = FALSE, colClasses = "character") %>% 
  bind_rows(read.csv(file.path(datamod.dir, "20210414_S_CHEM_PARAMETER_2020-append.csv"), stringsAsFactors = FALSE, colClasses = "character"))
...
```{r Check against sites table}

# Check against sites table??? (awaiting Charlie's response on site ID changes)
...
write.table(sample.all.FINAL, file = file.path(datamod.dir, "20210415_S_CHEM_HISTORY_SAMPLE_2020-append.csv"),sep=",", row.names = FALSE, na = "")
