...
# Mark validated_yn as "N" for all those above. Add NF flags and explanations to all those above except EMIL and MOHK sites.
...
result.all.form <- result.all %>%
  ...
  rename_all(function(x)
    paste0("CHR_", x)) %>% 
  rename(CHEM_PCODE = CHR_DEC_PCODE_FINAL)
...
# RESULT table
result.all.FINAL <- result.all.form %>% 
  select(CHEM_PCODE, CHR_SYS_SAMPLE_CDE, CHR_LAB_ANAL_METHOD_NAME, CHR_ANALYSIS_DATE, CHR_FRACTION, CHR_TEST_TYPE, CHR_LAB_MATRIX_CDE, 
         ...
         CHR_RESULT_TYPE_CDE, CHR_REPORTABLE_RESULT_IND, CHR_DETECT_FLAG, CHR_LAB_QUAL, CHR_INTERPRETED_QUAL, CHR_VALIDATED_IND, CHR_VALIDATOR_QUAL, 
         CHR_VALIDATOR_QUAL_EXPLN, CHR_VALIDATION_DATE, CHR_METHOD_DETECT_LIMIT, 
         ...
         CHR_QC_RPD_STATUS, CHR_SAMPLE_DEL_GRP)
...
Replace all NAs as blanks (run this???)
```{r}
...
Look at unique flags in 2001-2019 data
```{r}
