...
result.all.flagged <- result.all.flags.full
rm(result.all, qaqc.output.bound, result.all.flags.left, result.all.flags.full, result.all.flags.inner, result.all.flags.anti, result.sample.anti, result.sample.anti.sub, result.sample.anti.sub.mohk, dups)

```

Poplulate valididated indicator field 
```{r}

# Add "N" for those projects not run? Filter to N, N, missing flags...

```


Omit data to be excluded from final database
```{r}
# Mohawk DBP surrogate params?
```
...
result.pcodes <- left_join(result.all.flagged, pcode.join.lab, by = c("chemical_name", "fraction", "result_unit")) %>% 
  select(DEC_pcode_FINAL, chemical_name, everything())

result.pcodes.na <- result.pcodes %>% 
  filter(is.na(DEC_pcode_FINAL))

# result.pcodes.pcbpct <- result.pcodes %>% 
#   filter(grepl("*pcb"))
```
