...
sites <- read.csv(file.path(root.dir, "data/20191224_Site_Field_cleaned_final.csv"))
...
```{r}
# Subset sites table to just RIBS IDs and new site IDs
# sites.forjoin <- sites %>% 
#   select(BAS_LOC_RM, SH_OLD_RIBS_ID) %>% 
#   rename(sys_loc_code = SH_OLD_RIBS_ID,
#          SITE_ID = BAS_LOC_RM) %>% 
#   filter(sys_loc_code != "")
...
# equis.sample.siteids <- left_join(equis.sample, sites.forjoin, by = "sys_loc_code") %>% 
#   select(SITE_ID, everything())
equis.sample.siteids2 <- left_join(equis.sample, ribs.crosswalk, by = "sys_loc_code") %>% 
...
equis.sample.siteids.nomatch2 <- equis.sample.siteids2 %>% 
  filter(is.na(SBU_ID)) %>% 
  mutate(year = format(as.Date(sample_date, format="%m/%d/%Y"),"%Y")) %>% 
  select(year, everything())

# missing.ribsids <- equis.sample.siteids.nomatch %>% 
#   select(sys_loc_code) %>% 
#   distinct(sys_loc_code)

# missing.ribsids <- unique(equis.sample.siteids.nomatch$sys_loc_code) 
missing.ribsids2 <- unique(equis.sample.siteids.nomatch2$sys_loc_code) 
...
equis.location.missingribsids2 <- equis.location %>% 
  filter(SYS_LOC_CODE %in% missing.ribsids2,
         SYS_LOC_CODE != "QA_QC") %>% 
  select(SYS_LOC_CODE, LATITUDE, LONGITUDE, SURF_ELEV, ELEV_UNIT, LOC_NAME, LOC_DESC, LOC_COUNTY_CODE, DEC_REGION, LOC_MAJOR_BASIN, 
         NYS_DRAINAGE_BASIN_CODE, STREAM_MILE) %>% 
  mutate(lat_long = paste0(LATITUDE, "_", LONGITUDE))
...
# sites.for.match <- sites %>% 
#   mutate(lat_long = paste0(SH_LATITUDE, "_", SH_LONGITUDE)) %>% 
#   # filter(lat_long %in% equis.location.missingribsids$lat_long) %>% 
#   filter(SH_NAME %in% equis.location.missingribsids$LOC_NAME)
