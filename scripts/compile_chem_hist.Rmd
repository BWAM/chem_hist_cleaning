...
result.equis <- read_csv(file.path(tobecleaned.dir,  "equis_results_history.csv")) %>% 
  rename_all(tolower) %>%
  rename(sample_delivery_group = lab_sdg)
sample.equis <- read_csv(file.path(tobecleaned.dir, "equis_sample_history.csv")) %>% 
  rename_all(tolower) %>% 
...
result.2017 <- read.csv(file.path(tobecleaned.dir, "2017_chem_preqaqc_RESULT_complete_2020-11-06.csv"), stringsAsFactors = FALSE) %>%
  rename_all(tolower)
sample.2017 <- read.csv(file.path(tobecleaned.dir, "2017_chem_preqaqc_SAMPLE_complete_2020-11-06.csv"), stringsAsFactors = FALSE) %>%
  rename_all(tolower)
...
equis.join <- left_join(equis.sample.siteids, equis.result, by = "sys_sample_code")
