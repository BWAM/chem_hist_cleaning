...
# Same WG issue present as with 2016! But fewer.
...
# Fix WG matrix codes (rerun pre-qaqc?). Did not get sites assigned!
# Fix 
...
sample.2016 <- sample.2016.fix2
result.2016 <- result.2016.fix
...
result.2016.unique <- lapply(result.2016, unique)
...
sample.2016.fix.unique <- lapply(sample.2016.fix, unique)
...
result.2016.unique <- lapply(result.2016.fix, unique)
sample.2016.unique <- lapply(sample.2016.fix, unique)

# Overwrite master DFs and clean up.
sample.2016 <- sample.2016.fix
result.2016 <- result.2016.fix
 
rm(sample.2016.fix, result.2016.fix, sample.2016.fix.unique)
```
...
# Clean overwrite masters.

sample.equis <- sample.equis.keep
result.equis <- result.equis.keep
...
rm(sample.equis.keep, result.equis.keep, sample.equis.archive, result.equis.archive, sample.equis.orphan)
rm(equis.join, sample.equis.nomatch, result.equis.nomatch)
```

```{r Sample type, matrix code, and misc cleaning}
sample.equis.unique <- lapply(sample.equis, unique)
result.equis.unique <- lapply(result.equis, unique)

sample.equis.fix <- sample.equis %>% 
  mutate()
...
# Join all datasets and review for consistency
